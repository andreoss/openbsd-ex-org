...
#+begin_src shell
  QEMU_NETWORK=(
      ...
      -device virtio-net,netdev=mn0,mac=de:ad:de:ad:de:01
  )
#+end_src
...
** Build Emacs
 #+begin_src sh :results output
   ...
   git pull
   gmake clean
   AUTOCONF_VERSION=2.65 ./autogen.sh
   CC=egcc MAKEINFO='/usr/local/bin/gmakeinfo' ./configure --prefix=$HOME/.local --with-json --without-x 
   gmake
   gmake install
 #+end_src
