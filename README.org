...
1. 6.9
2. 7.0 (Snapshot)
   In oreder to build 7.0 image change this parameters
...
#+begin_src emacs-lisp
  ...
  (defun a/not-contains? (needle haystack)
    (assert (not (cl-search needle haystack))))
        #+end_src
...
*** Add RSA key
#+name: ssh-key
#+begin_src emacs-lisp :post assert!(*this*, '(a/not-contains? "The agent has no identities." block))
...
#+begin_src shell :post assert!(*this*, '(a/contains? "/usr/local/bin/ratpoison" block))
which ratpoison
#+end_src
...
#+begin_src shell :post assert!(*this*, '(a/contains? "/usr/local/bin/urxvt" block))
which urxvt
#+end_src
...
#+call: pkg_add("rsync--")
#+begin_src shell :post assert!(*this*, '(a/contains? "/usr/local/bin/rsync" block))
which rsync
#+end_src
